...
export default function ExpensesPage() {
  ...
  const deleteExpense = async (expObj: Expense) => {
    if (!confirm("Delete this expense?")) return
    // expenses state is reversed; find position in original (backend) order
    const revIdx = expenses.findIndex(e =>
      e.Date === expObj.Date && e.Amount === expObj.Amount &&
      e.Description === expObj.Description && e.PaidTo === expObj.PaidTo
    )
    if (revIdx === -1) { flash("⚠ Could not find expense"); return }
    const originalIdx = expenses.length - 1 - revIdx
    try {
      await fetch(`${API}/api/expenses/${originalIdx}`, { method: "DELETE" })
      flash("✅ Deleted"); load()
  ...
  const handleCsvImport = async (file: File) => {
    ...
    try {
      ...
      const r = await fetch(`${API}/api/expenses/import-csv`, {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ csv_text: text })
      }).then(r => r.json())
      const d = r.data ?? r
      flash(`✅ Imported ${d.added ?? 0} expenses${d.skipped ? `, skipped ${d.skipped}` : ""}`)
      load()
    } catch { flash("⚠ CSV import failed") }
      ...
      <div className="space-y-5">
        <div className="flex items-center justify-between flex-wrap gap-3">
          ...
          <div className="flex gap-2 flex-wrap">
            <a href={`${API}/api/expenses/export-csv`} download
        ...
        <div className="bg-blue-50 border border-blue-100 rounded-xl px-4 py-3 text-xs text-blue-700 flex items-start gap-2">
          ...
          <span><strong>CSV Import columns:</strong> Date (YYYY-MM-DD), Category, Description, Amount, PaidTo, PaidBy, PaymentMode, Notes</span>
        </div>
                  ...
                  {filtered.map((e, i) => (
                    <tr key={i} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        <button onClick={() => deleteExpense(e)} className="p-1.5 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors">
